...
let nextBoss = ['ABC', "10:00", 3];
let hourIndex = time.indexOf(nextBoss[1]);
let rectX = rectPositionX[nextBoss[2]];
let rectY = rectPositionY[hourIndex];
rows.push(`<rect x="0" y="${rectY}" width="990" height="60"
style="stroke:red;stroke-width:2;fill-opacity: 0.1" />`);
rows.push(`<rect x="${rectX}" y="0" width="130" height="480"
style="stroke:red;stroke-width:2;fill-opacity: 0.1" />`);
let data = rows.join('\n');
svg = svg + data + '</svg>';
